...
const taskBorderColors: Record<string, string> = {
  ...
  inprogress: "border-l-emerald-500",
...
export const MyWorkSheet = () => {
  ...
  // Reset collections when project changes
  useEffect(() => {
    setAllTasks([]);
    setAllIssues([]);
    setAllTickets([]);
          ...
          {activeTasks.map((task) => {
            return (
              <div
                ...
                className={cn(
                  "border-l-4 bg-muted/80 hover:bg-muted/90 border border-accent border-l-solid p-4 rounded-md transition-colors cursor-pointer space-y-3",
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 pt-1 border-t border-accent/20">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-medium capitalize px-2 py-0.5 rounded-md border border-accent bg-card text-white/90 flex items-center gap-1.5">
                      {statusIconsNoColors[task.status] || <Ellipsis className="w-3.5 h-3.5" />}
                      {task.status}
                    </span>
                    <div className="flex items-center gap-1 text-[9px] text-muted-foreground capitalize">
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 pt-1 border-t border-accent/20">
                  <div className="flex items-center gap-2">
                    ...
                    {issue.severity && (
                      <span className={cn(
                        "text-[8px] uppercase tracking-wider px-1.5 py-0.5 rounded font-semibold",
                        severityConfig[issue.severity]?.class || "bg-accent text-muted-foreground"
                      )}>
                        {issue.severity}
                      </span>
                    )}
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        ...
        <div className="p-6 border-b border-accent/60 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-1 border border-accent/50 bg-muted/40 p-0.5 rounded-lg w-full">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={cn(
                  "flex items-center justify-center gap-1 text-[11px] py-1.5 rounded-md font-medium transition-all duration-200 cursor-pointer w-full",
                  activeTab === tab.id
                    ? "bg-sidebar text-primary shadow-xs border border-accent"
