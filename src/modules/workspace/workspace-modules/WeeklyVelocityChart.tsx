...
export const WeeklyVelocityChart = ({
  projectId,
  data: providedData,
  ...
  const data = (providedData || queryData) as
    | { day: string; tasks: number; issues: number }[]
    | undefined;
  ...
  const totalThisWeek = data.reduce(
    (acc, curr) => acc + curr.tasks + curr.issues,
    0,
  );
      ...
      <Card className="border shadow-sm dark:bg-accent/20 bg-card dark:border-accent border-accent/50 overflow-hidden h-[340px]">
              ...
              <Tooltip>
                ...
                >
                  ...
                  <p className="leading-relaxed text-muted-foreground">
                    Tracks the number of completed tasks and closed issues on a
                    daily basis (Monday to Sunday) during the current week to
        ...
        <CardContent className="flex flex-col items-center justify-center h-[220px] text-center p-6">
          ...
          <p className="text-[10px] text-muted-foreground max-w-[200px] mt-1">
            Complete tasks and resolve issues to see your weekly performance
            trends.
          </p>
        </CardContent>
      </Card>
          ...
          <TooltipProvider delayDuration={100}>
            <Tooltip>
              ...
              >
                ...
                <p className="leading-relaxed text-muted-foreground">
                  Tracks the number of completed tasks and closed issues on a
                  daily basis (Monday to Sunday) during the current week to show
        ...
        <ChartContainer config={chartConfig} className="h-[220px] w-full">
          ...
          >
            <CartesianGrid
              vertical={false}
              strokeDasharray="3 3"
              className="stroke-primary/30! dark:stroke-muted!"
            />
            <XAxis
              dataKey="day"
              tickLine={false}
