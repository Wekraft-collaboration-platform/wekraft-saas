...
export async function verifyProjectAccess(
  ...
  try {
    ...
    const user = await convex.query(api.user.getUserByClerkToken, {
      clerkToken: clerkUserId,
    });
    if (!user)
      return { error: "User not found in Wekraft database", status: 404 };

    // 2. Fetch project membership/ownership using the fetched user's ID
    const permissions = await convex.query(
      api.project.getProjectPermissionsById,
      {
    ...
    if (
      !permissions ||
      (!permissions.isOwner && !permissions.isAdmin && !permissions.isMember)
    ) {
      return {
