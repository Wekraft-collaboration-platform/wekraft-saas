...
const features = [
  ...
  {
    name: "GitHub bi-directional sync",
    wekraft: "check",
    asana: "minus",
    jira: "check",
    linear: "check",
  ...
  {
    name: "Heatmap / burnout detection",
    wekraft: "check",
    asana: "x",
    jira: "x",
    linear: "x",
  ...
  {
    name: "Built-in developer analytics",
    wekraft: "check",
    asana: "x",
    jira: "minus",
    linear: "check",
...
function renderIcon(status: string, isWekraft = false) {
  if (status === "check") {
    return (
      <div
        className={`w-6 h-6 rounded-full flex items-center justify-center mx-auto ${isWekraft ? "bg-blue-500/20 border border-blue-500/30" : "bg-white/5 border border-white/10"}`}
      >
        <Check
          className={`w-3.5 h-3.5 ${isWekraft ? "text-blue-400" : "text-white/40"}`}
        />
      </div>
    );
  }
    ...
    <div className="w-full font-sans">
      ...
      >
        ...
        <p className="text-neutral-400 text-base max-w-3xl leading-relaxed">
          Jira is powerful but bloated. Asana is too generic for developers.
          Linear is fast but rigid. We built WeKraft to give software
          engineering teams the{" "}
          <strong className="text-white font-medium">blazing speed</strong> they
          crave, with the{" "}
          <strong className="text-white font-medium">AI-driven depth</strong>{" "}
          they actually need.
        </p>
      </motion.div>
        ...
        {keys.map((key, i) => {
          ...
          return (
            ...
            >
              ...
              <p
                className={cn(
                  "text-xs font-semibold mb-2 relative z-10",
                  isWe ? "text-blue-300" : "text-white/50",
                )}
              >
                {competitors[i]}
              </p>
              <p
                className={cn(
                  ...
                  isWe ? "text-white drop-shadow-md" : "text-white/40",
                )}
              >
                {pct}%
              </p>
              <p className="text-[10px] uppercase tracking-wider font-mono text-white/30 mt-2 relative z-10">
                match
              </p>
            </motion.div>
          );
        ...
        <div className="min-w-[760px]">
          ...
          <div className="grid grid-cols-8 gap-2 pb-4 mb-2 border-b border-white/8">
            ...
            {competitors.map((c, i) => (
              <div
                key={c}
                className={`text-center text-xs font-semibold ${i === 0 ? "text-blue-400" : "text-white/30"}`}
              >
                {c}
              </div>
            ))}
          ...
          <div className="flex items-center gap-6 mt-6 pt-6 border-t border-white/8 text-xs text-white/25">
            ...
            <div className="flex items-center gap-2">
              <Minus className="w-3.5 h-3.5 text-white/25" /> Partial / plugin
              needed
            </div>
            <div className="flex items-center gap-2">
            ...
            <span className="ml-auto">
              *Based on default out-of-the-box capabilities, no add-ons.
            </span>
          </div>
        </div>
      </div>
