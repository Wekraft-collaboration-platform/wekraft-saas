...
const allTools = {
    getMemberWorkload: tool({
        description: "Get the current tasks and issues workload breakdown for all project members.",
        inputSchema: z.object({
            projectId: z.string().describe("The ID of the project"),
        }),
        execute: async ({ projectId }) => {
            console.log("Kaya Tool: getMemberWorkload called for", projectId);
            return await convex.query(api.teamspaceAgents.getMemberWorkload, { projectId });
        },
    }),
    ...
    getProjectInsights: tool({
        ...
        execute: async ({ projectId }) => {
            ...
            return await convex.query(api.teamspaceAgents.getProjectInsights, { projectId: projectId as any });
        },
    }),

    getTasksSummary: tool({
        description: "Get an AI-optimized summary of project tasks, prioritizing critical, active, completed, and blocked tasks.",
        inputSchema: z.object({
            projectId: z.string().describe("The ID of the project"),
        }),
        execute: async ({ projectId }) => {
            console.log("Kaya Tool: getTasksSummary called for", projectId);
            return await convex.query(api.teamspaceAgents.getTasksSummary, { projectId: projectId as any });
        },
    }),
    ...
    getIssuesSummary: tool({
        ...
        execute: async ({ projectId }) => {
            ...
            return await convex.query(api.teamspaceAgents.getIssuesSummary, { projectId: projectId as any });
        },
    }),

    getProjectVelocity: tool({
        description: "Get project throughput velocity, average cycle times, and the 4-week trend.",
        inputSchema: z.object({
            projectId: z.string().describe("The ID of the project"),
        }),
        execute: async ({ projectId }) => {
            console.log("Kaya Tool: getProjectVelocity called for", projectId);
            return await convex.query(api.teamspaceAgents.getProjectVelocity, { projectId: projectId as any });
        },
    }),

    getSprintHistory: tool({
        description: "Get sprint completion analytics and goal history for completed and active sprints.",
        inputSchema: z.object({
            projectId: z.string().describe("The ID of the project"),
        }),
        execute: async ({ projectId }) => {
            console.log("Kaya Tool: getSprintHistory called for", projectId);
            return await convex.query(api.teamspaceAgents.getSprintHistory, { projectId: projectId as any });
        },
    }),
};
...
You assist the team inside the teamspace chat by analyzing project metrics, task statuses, issues, team workloads, sprints, and velocity.

Your Behaviour:
- Use your tools to fetch accurate details from the project whenever the user asks for tasks, issues, workload, velocity, sprints, or overall insights.
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!projectId || !messages) {
            return new Response("projectId and messages are required", { status: 400 });
        }

        // Strict rate limit per project (3 requests per minute for the whole project team)
        const limitKey = projectId;
        const { success, limit, reset, remaining } = await kayaRatelimit.limit(limitKey);
        if (!success) {
            return new Response("Too many requests. The project rate limit is 3 requests per minute.", {
                status: 429,
                headers: {
        ...
        console.error("Error in Kaya teamspace route:", error);
