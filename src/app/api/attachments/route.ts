...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log(`Processing attachment: ${file.name}, Size: ${file.size}, Type: ${file.type}`);

    // Validation: Max size 10MB for attachments
    if (file.size > 10 * 1024 * 1024) {
        console.warn("File too large");
        return NextResponse.json({ error: "File too large. Max 10MB allowed for attachments." }, { status: 400 });
    }
